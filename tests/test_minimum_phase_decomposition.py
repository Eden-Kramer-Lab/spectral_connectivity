...
def _expands_dims(array):
    return array[np.newaxis, :, np.newaxis, np.newaxis]
...
def test__get_causal_signal_removes_roots_outside_unit_circle():
    ...
    transfer_function = _expands_dims(transfer_function)

    expected_causal_signal = np.ones(
        (1, n_fft_samples, n_signals, n_signals), dtype=np.complex)

    causal_signal = _get_causal_signal(linear_predictor=transfer_function)
...
def test__get_causal_signal_preserves_roots_inside_unit_circle():
    ...
    linear_coef = irfft(expected_transfer_function,
                        n=len(expected_transfer_function))
    linear_coef[0] *= 0.5

    expected_causal_signal = _expands_dims(fft(linear_coef))
    causal_signal = _get_causal_signal(linear_predictor=transfer_function)
...
def test_minimum_phase_decomposition():
    ...
    expected_minimum_phase_factor = np.r_[
        _expands_dims(transfer_function), _expands_dims(transfer_function2)]
