...
def test_coherence_magnitude_bounds():
    ...
    assert np.all(valid_values >= -eps_margin), f"Coherence magnitude below 0: {valid_values.min()}"
    assert np.all(valid_values <= 1 + eps_margin), f"Coherence magnitude above 1: {valid_values.max()}"
...
def test_imaginary_coherence_bounds():
    ...
    assert np.all(valid_values >= -eps_margin), f"Imaginary coherence below 0: {valid_values.min()}"
    assert np.all(valid_values <= 1 + eps_margin), f"Imaginary coherence above 1: {valid_values.max()}"
