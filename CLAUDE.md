...
- **GPU/CPU Abstraction**: Uses `xp` namespace (numpy or cupy) controlled by `SPECTRAL_CONNECTIVITY_ENABLE_GPU` environment variable. `_backend.py` selects it once at import; modules import `xp` and the FFT/signal/sparse routines from there, never from numpy/scipy/cupy directly
